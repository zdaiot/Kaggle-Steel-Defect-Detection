...
class solver():
    ...
    def forward(self, images):
        ...
        Return:
            classes_predict: [batch_size, class_num]，One-hot数据
        '''
        images = images.to(self.device)
        classes_predict = self.model(images)
        return classes_predict

    def cal_loss(self, classes, classes_predict, criterion):
        ''' 根据真实类标和预测出的类标计算损失
        
        Args:
            classes: [batch_size, class_num]，真实类标，One-hot数据
            classes_predict: [batch_size, class_num, 1, 1]，预测出的数据
            criterion: 使用的损失函数
        ...
        return criterion(classes_predict, classes)
